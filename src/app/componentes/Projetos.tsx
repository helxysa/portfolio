...
export default function Projetos() {
    ...
    const projetosExibidos = mostrarTodos 
        ? projetosFiltrados 
        : projetosFiltrados.slice(0, projetosPorPagina)

    return (
        <section className="w-full py-8 sm:py-12 md:py-16 bg-gradient-to-b from-[#1e1e1e] to-gray-900" id='projetos'>
            ...
            <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8">
                ...
                <div className="flex justify-center mt-8">
                    <button
                        ...
                        className="px-6 py-3 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-lg hover:from-purple-700 hover:to-purple-800 transition-all duration-300 shadow-lg hover:shadow-purple-600/50"
                    >
                        {mostrarTodos ? 'Mostrar Menos' : 'Ver Todos os Projetos'}
                    </button>
                </div>
            </div>
        </section>
...
const ProjetoCard = ({ projeto, index }: { projeto: Projeto, index: number }) => (
    <div className="group bg-gray-800/80 backdrop-blur-sm rounded-lg sm:rounded-xl md:rounded-2xl overflow-hidden shadow-md sm:shadow-lg md:shadow-xl transform transition-all duration-500 hover:scale-[1.02] hover:shadow-purple-500/30 border border-gray-700/50">
        <div className="relative h-36 xs:h-40 sm:h-44 md:h-56 overflow-hidden">
            {projeto.image ? (
                <Image
                    ...
                    width={380}
                    height={214}
                    className="object-cover object-top w-full h-full transform scale-100 group-hover:scale-110 transition-transform duration-700 ease-out"
                    quality={90}
                    priority={index < 2}
        ...
        <div className="p-3 xs:p-4 sm:p-6 md:p-8">
            ...
            <p className="text-xs xs:text-sm sm:text-base text-gray-300 mb-2 xs:mb-3 sm:mb-4 md:mb-6 line-clamp-2 sm:line-clamp-3 group-hover:text-gray-200 transition-colors duration-300">
                {projeto.descricao}
            </p>
            ...
            <div className="space-y-2 xs:space-y-3 sm:space-y-4">
                ...
                <div className="flex gap-1.5 xs:gap-2 sm:gap-3 md:gap-4">
                    <a 
                        ...
                        className="flex-1 bg-gray-700/80 text-white text-center py-1.5 xs:py-2 sm:py-2.5 md:py-3 rounded-md sm:rounded-lg text-[10px] xs:text-xs sm:text-sm md:text-base font-medium hover:bg-gray-600 transition-all duration-300 shadow-lg hover:shadow-gray-700/50 backdrop-blur-sm"
                    >
                        GitHub
                    </a>
                    {projeto.mostrarSite && (
                        <a 
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex-1 bg-gradient-to-r from-purple-600 to-purple-700 text-white text-center py-1.5 xs:py-2 sm:py-2.5 md:py-3 rounded-md sm:rounded-lg text-[10px] xs:text-xs sm:text-sm md:text-base font-medium hover:from-purple-700 hover:to-purple-800 transition-all duration-300 shadow-lg hover:shadow-purple-600/50"
                        >
                            Visitar Site
                        </a>
                    )}
